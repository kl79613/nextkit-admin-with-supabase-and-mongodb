...
export const ProductPerformance = () => {
  ...
  function handleInput() {
    if (inputValue) {
      const modifiedProducts = allproducts.filter((item: any) =>
  ...
  function handleRenderedProduct() {
    const products = fixedProducts.slice(
      (currentpagination - 1) * 4,
      4 * currentpagination
    );
  ...
  function handleFilterProduct() {
    if (status == "All") {
      ...
      const modifiedProducts = fixedProducts.filter(
            ...
            <div className="flex gap-4">
              ...
              >
                <DropdownItem
                  ...
                  className={`${
                    status == "All" ? "bg-lightprimary text-primary" : ""
                  }`}
                >
                  All
                </DropdownItem>
                <DropdownItem
                  ...
                  className={`${
                    status == "shipped" ? "bg-lightprimary text-primary" : ""
                  }`}
                >
                  shipped
                </DropdownItem>
                <DropdownItem
                  ...
                  className={`${
                    status == "pending" ? "bg-lightprimary text-primary" : ""
                  }`}
                >
                  pending
                </DropdownItem>
                <DropdownItem
                  ...
                  className={`${
                    status == "delivered" ? "bg-lightprimary text-primary" : ""
                  }`}
                >
                  delivered
                </DropdownItem>
                  ...
                  <div className="flex items-center divide-x border border-border rounded-md">
                    ...
                    <button
                      onClick={() => {
                        setCurrentPagination((prev) => {
                          if (
                            prev < Math.ceil(fixedProducts.length / itemsCount)
                          ) {
                            return prev + 1;
                          } else {
                            return prev;
                      ...
                      disabled={
                        currentpagination ===
                        Math.ceil(fixedProducts.length / itemsCount)
                      }
                      className={`py-1.5 px-4 text-sm font-medium hover:bg-lightprimary hover:text-primary ${
                        currentpagination ===
                        Math.ceil(fixedProducts.length / itemsCount)
