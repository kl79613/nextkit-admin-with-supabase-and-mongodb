  ...
  async function handleOrders() {
    try {
      ...
      const totalSales = result.data.reduce(
        (reducer: number, item: { quantity: any }) => {
            ...
            <CardBox className="flex flex-col">
              <div className="flex items-center justify-between">
                <div className="flex flex-col">
                  <h5 className="text-dark text-base font-bold">
                    {item.metric}
                  </h5>
                  <span className="text-[13px]">{item.subTitle}</span>
                </div>
                <span
                  className={`flex size-11 items-center justify-center rounded-full ${item.bgColor}`}
                >
                  <Icon
                    icon={item.icon}
              ...
              <h5 className="text-dark mt-3 text-xl font-semibold">{` ${item.metric.toLocaleLowerCase().includes("sales") ? "$" : ""} ${item.value}`}</h5>
            </CardBox>
          </div>
        ))}
      </div>
